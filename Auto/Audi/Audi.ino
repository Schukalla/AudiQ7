...
//Pins
#define OUTPUT_PIN 15  // D22 //output
#define INPUT_PIN1 16  // D23 //input
#define INPUT_PIN2 17  // RX2 (GPIO16) //input
...
void processGamepad(ControllerPtr ctl) {
    ...
    if (remgang == 1) {
        ...
        motorController.motorABackward(speed);
    } else {
        motorController.motorStop();
    }
    ...
    if (var < -40) {
        Serial.printf("unter 40");
        var *= -1;
        motorController.motorBBackward(var);
    } else if (axisRX > 40) {      
        Serial.printf("über 40");
        motorController.motorBForward(var);
    }
}
